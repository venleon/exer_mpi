...
! initial conditions
allocate(s%hzero(1:s%nx+1,1:s%ny+1))
allocate(s%eta(1:s%nx+1,1:s%ny+1))
allocate(s%etan(1:s%nx+1,1:s%ny+1))
allocate(s%h(1:s%nx+1,1:s%ny+1))
allocate(s%wet(1:s%nx+1,1:s%ny+1))
allocate(s%u(1:s%nx+1,1:s%ny+1))
allocate(s%un(1:s%nx+1,1:s%ny+1))
allocate(s%v(1:s%nx+1,1:s%ny+1))
allocate(s%vn(1:s%nx+1,1:s%ny+1))
...
DO k = 1,s%nx+1
 s%hzero(1,k) = -10.0
 s%hzero(s%ny+1,k) = -10.0
END DO

DO j = 1,s%ny+1
 s%hzero(j,1) = -10.0
 s%hzero(j,s%nx+1) = -10.0
END DO

DO j = 1,s%ny+1
DO k = 1,s%nx+1
  s%eta(j,k) = -MIN(0.0,s%hzero(j,k))
  s%etan(j,k) = s%eta(j,k)
END DO
END DO
!XXXXXXXXXXXXXXXXXXX
...
DO j = 1,s%ny+1
DO k = 1,s%nx+1
  s%h(j,k) = s%hzero(j,k)+s%eta(j,k)
! wet = 1 defines "wet" grid cells
! wet = 0 defines "dry" grid cells
  s%wet(j,k) = 1
  if(s%h(j,k) < par%hmin) s%wet(j,k) = 0
  s%u(j,k) = 0.
  s%un(j,k) = 0.
  s%v(j,k) = 0.
  s%vn(j,k) = 0.
