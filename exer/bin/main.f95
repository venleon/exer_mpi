...
write(*,*) 'call space_distribute'
	s => slocal
	call space_distribute_space(sglobal,slocal,par)
#endif
	call printit(sglobal,slocal,par,0,'after space_distribute_space')

if (xmaster) then
  write(*,*) 'Building Grid and Bathymetry and....'
  write(*,*) 'Distributing grid data across procs'     
...
!CALL shapiro
CALL inc1(s,par)
call printit(sglobal,slocal,par,n,'after inc1')

! data output
IF(MOD(n,par%nout)==0)THEN
  DO j = 1,s%ny
    WRITE(10,'(201F12.6)')(s%eta(j,k),k=1,s%nx)
